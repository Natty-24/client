  ...
  return(
    <Link href={href}>
      ...
      >
       ...
       <span className={`${isCollapsed ? "hidden" : "black"} font-medium text-gray-700`}>
        {label}
       </span>
      </div>
    </Link>
  ...
  return (
    <div className={sidebarClassNames}>
        ...
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            <div>logo</div>
            <h1 className={` ${isSidebarCollapsed ? "hidden" : "block"}  font-extrabold text-2xl`}>SadorStock</h1>
        ...
        <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
            <p className='text-center text-xs text-gray-500'>&copy; 2025 SadorStock</p>
        </div>
    </div>
  )
}
